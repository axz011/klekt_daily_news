...
- 从多个主流媒体 RSS 抓取伊朗、乌克兰相关的新闻（无需 API key）
- 通过关键词过滤确保只收集相关新闻
...
def matches_category_keywords(text, keywords):
    """检查文本是否包含指定类别的关键词"""
    if not text:
        return False
...
def is_relevant_news(item):
    ...
    # 检查是否属于伊朗类别且包含伊朗关键词
    if 'iran' in category and matches_category_keywords(combined_text, IRAN_KEYWORDS):
        return True
    
    # 检查是否属于乌克兰类别且包含乌克兰关键词
    if 'ukraine' in category and matches_category_keywords(combined_text, UKRAINE_KEYWORDS):
        return True
    
    # 即使分类不明确，如果内容同时出现类别关键词和相关关键词也应收入
    if matches_category_keywords(combined_text, IRAN_KEYWORDS):
        return True
    
    if matches_category_keywords(combined_text, UKRAINE_KEYWORDS):
        return True
...
def build_email_body(items):
    lines = []
    lines.append("日报：伊朗、乌克兰要闻（RSS 源，自动生成）\n")
