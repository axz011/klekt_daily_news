...
def collect_top_items(limit=20):
    ...
    items.sort(key=lambda x: (x.get('category'), x.get('published') or datetime.min, x.get('importance', 0)), reverse=True)
    return items[:limit]
...
def main():
    ...
    subject = f"每日AI要闻 — {datetime.now(BJT).strftime('%Y-%m-%d')}"
